...
app.get('/health', (req, res) => {
  res.json({ success: true, ready: client.isReady(), guildId: GUILD_ID });
});
...
app.post('/assignRoles', async (req, res) => {
  try {
    ...
    if (!Array.isArray(roleIds)) return res.status(400).json({ success: false, error: 'roleIds fehlen' });
...
app.post('/dismiss', async (req, res) => {
  try {
    ...
    for (const roleId of roles) {
      // Besucher-/Ex-Member-Rolle NICHT entfernen
      if (EX_MEMBER_ROLE_ID && roleId === EX_MEMBER_ROLE_ID) continue;
    ...
    // Besucher-/Ex-Member-Rolle sicher setzen
